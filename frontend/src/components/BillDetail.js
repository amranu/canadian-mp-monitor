...
function BillDetail() {
  ...
  const loadRelatedVotes = async () => {
    try {
      ...
      // Load votes and filter for ones related to this bill
      const votesData = await parliamentApi.getVotes(200, 0); // Get more votes to find related ones
      const billUrl = `/bills/${session}/${number}/`;
      const related = votesData.objects.filter(vote => vote.bill_url === billUrl);
      setRelatedVotes(related);
    } catch (error) {
